...
const STARTER_PROMPTS = [
  {
    ...
    subtitle:
      "10 days &bull; Tokyo, Kyoto, Nara, Osaka &bull; Temples & Gastronomy",
    prompt:
  ...
  },
  {
    ...
    subtitle:
      "7 days &bull; Zurich, Interlaken, Zermatt &bull; Panoramas & Trains",
    prompt:
  ...
  },
  {
    ...
    subtitle:
      "8 days &bull; Rome, Florence, Amalfi Coast &bull; Architecture & Wine",
    prompt:
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center justify-center text-center py-2 sm:py-4 px-2 sm:px-4 font-body">
      ...
      <p className="text-sm sm:text-base text-[#1F1E1E] dark:text-[#E0E0E0] max-w-lg leading-relaxed mb-8 sm:mb-10 font-medium">
        Tell TripVerse where you want to go, what you want to experience, and
        how you like to travel. We will map your itinerary into an interactive
        trip universe.
      </p>

      {/* Starter Prompts Grid */}
      <div className="w-full text-left space-y-3">
        <div className="text-[10px] font-black uppercase tracking-widest text-[#1F1E1E] dark:text-[#CCCCCC] px-1 flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 bg-[#1F1E1E] dark:bg-white" />
          <span>Curated Voyage Inspirations</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          {STARTER_PROMPTS.map((item, idx) => (
            <button
              ...
              className="p-4 bg-white dark:bg-[#1E1E1E] border-2 border-[#1F1E1E] dark:border-[#555555] shadow-tactile btn-tactile text-left flex flex-col justify-between group cursor-pointer rounded-none relative overflow-hidden"
            >
              <div>
                <div className="flex items-center justify-between gap-2">
                  <span className="font-black text-xs uppercase tracking-tight text-[#1F1E1E] dark:text-white group-hover:text-black dark:group-hover:text-white">
                    {item.title}
                  </span>
                  <div className="w-5 h-5 bg-[#F5F5F5] dark:bg-[#2A2A2A] border border-[#1F1E1E] dark:border-[#555555] flex items-center justify-center shrink-0 shadow-tactile-sm">
                    <ArrowRight className="w-3 h-3 text-[#1F1E1E] dark:text-white group-hover:translate-x-0.5 transition-transform" />
                  </div>
                </div>
                <div
                  className="text-[11px] text-[#1F1E1E]/85 dark:text-[#BBBBBB] mt-2 line-clamp-2 font-medium leading-snug"
